...
**📁 工作区是一个可整体搬运的单元。** Agent 配置、技能、会话历史、项目文档全是 `.halo/` 下的文件 —— 没有隐藏记忆，没有不透明状态。因为整个 Agent 都活在一个目录里，你分享的单位是*整个工作区*，而不是单个技能：拷贝 `.halo/`（或 `git clone`）后，对方拿到的是一个完整、可运行的 Agent —— 人设、全部技能、配置俱全，拿到即可开始工作。这也是自我进化得以成立的基础：进化 Agent 改写的是工作区里真实的文件，在沙箱里 dry-run，审核通过后合并。

...
export function WecomSettings() {
  ...
  const reload = useCallback(() => {
    setLoading(true)
    api.wecom.listAccounts().then((r) => setAccounts(r.accounts)).catch((err) => {
      console.error('[wecom] list failed:', err)
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between border-b border-[var(--border)] px-4 py-3">
        <div>
          <h2 className="text-sm font-medium text-[var(--foreground)]">{t('wecom.title')}</h2>
          <p className="text-[11px] text-[var(--muted-foreground)]">{t('wecom.desc')}</p>
        </div>
        <button
...
function AccountRow(props: {
  ...
  async function save() {
    setBusy(true)
    try {
      await api.wecom.updateAccount(account.accountId, { label, workspacePath, accessLevel, language })
      onSaved()
    } catch (err) {
      alert(t('wecom.saveFailed', { error: err instanceof Error ? err.message : String(err) }))
  ...
  async function toggle() {
    setBusy(true)
    try {
      await api.wecom.updateAccount(account.accountId, { enabled: !account.enabled })
      onToggled()
    } catch (err) {
      alert(t('wecom.switchFailed', { error: err instanceof Error ? err.message : String(err) }))
    } finally { setBusy(false) }
  }

  async function remove() {
    if (!(await confirmAction(t('wecom.confirmDelete', { name: account.botId })))) return
    setBusy(true)
    try {
      await api.wecom.deleteAccount(account.accountId)
      onDeleted()
    } catch (err) {
      alert(t('wecom.deleteFailed', { error: err instanceof Error ? err.message : String(err) }))
    } finally { setBusy(false) }
  }
    ...
    return (
      <li className="px-4 py-3">
        <div className="space-y-2">
          <div>
            <label className="text-[10px] text-[var(--muted-foreground)]">{t('wecom.label')}</label>
            <input
              value={label}
              onChange={(e) => setLabel(e.target.value)}
              className="mt-0.5 w-full rounded border border-[var(--border)] bg-[var(--card)] px-2 py-1 text-xs"
              placeholder={t('wecom.labelPlaceholder')}
            />
          </div>
          <div>
            <label className="text-[10px] text-[var(--muted-foreground)]">{t('wecom.workspace')}</label>
            <input
              value={workspacePath}
          ...
          <div>
            <label className="text-[10px] text-[var(--muted-foreground)]">{t('wecom.accessLevel')}</label>
            <select
              value={accessLevel}
              onChange={(e) => setAccessLevel(e.target.value as 'full' | 'workspace' | 'readonly' | 'observer')}
              className="mt-0.5 w-full rounded border border-[var(--border)] bg-[var(--card)] px-2 py-1 text-xs"
            >
              <option value="readonly">{t('wecom.readonly')}</option>
              <option value="workspace">{t('wecom.wsWrite')}</option>
              <option value="full">{t('wecom.full')}</option>
            </select>
          </div>
          ...
          <div className="flex gap-2">
            ...
            >
              <Check className="h-3 w-3" /> {t('wecom.save')}
            </button>
            <button
            ...
            >
              <X className="h-3 w-3" /> {t('wecom.cancel')}
            </button>
          </div>
        </div>
      </li>
    ...
    <li className="flex items-center gap-3 px-4 py-3">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <span className="text-xs font-medium text-[var(--foreground)]">{account.label || account.botId}</span>
          <span className={cn(
        ...
        <p className="mt-0.5 truncate font-mono text-[10px] text-[var(--muted-foreground)]">
          bot:{account.botId} → {account.workspacePath}
          {account.workspaceMissing && <span className="ml-1 text-red-400">{t('wecom.pathMissing')}</span>}
        </p>
      </div>
      <div className="flex items-center gap-1">
        <button
          onClick={toggle}
          disabled={busy}
          title={account.enabled ? t('wecom.disable') : t('wecom.enable')}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-sm font-medium text-[var(--foreground)]">{t('wecom.addTitle')}</h3>
        ...
        <div className="space-y-3">
          <div>
            <label className="text-[11px] text-[var(--muted-foreground)]">{t('wecom.botIdLabel')}</label>
            <input
              value={botId}
              onChange={(e) => setBotId(e.target.value)}
              className="mt-1 w-full rounded border border-[var(--border)] bg-[var(--card)] px-2 py-1.5 font-mono text-xs"
              placeholder="wecombot_xxxxxxx"
            />
          </div>
          <div>
            <label className="text-[11px] text-[var(--muted-foreground)]">{t('wecom.secretLabel')}</label>
            <input
              type="password"
              value={secret}
          ...
          <div>
            <label className="text-[11px] text-[var(--muted-foreground)]">{t('wecom.bindWorkspace')}</label>
            <input
              value={workspacePath}
              onChange={(e) => setWorkspacePath(e.target.value)}
              className="mt-1 w-full rounded border border-[var(--border)] bg-[var(--card)] px-2 py-1.5 font-mono text-xs"
          ...
          <div>
            <label className="text-[11px] text-[var(--muted-foreground)]">{t('wecom.nameOptional')}</label>
            <input
              value={label}
              onChange={(e) => setLabel(e.target.value)}
              className="mt-1 w-full rounded border border-[var(--border)] bg-[var(--card)] px-2 py-1.5 text-xs"
              placeholder={t('wecom.namePlaceholder')}
            />
          </div>
          <div>
            <label className="text-[11px] text-[var(--muted-foreground)]">{t('wecom.accessLevel')}</label>
            <select
              value={accessLevel}
              onChange={(e) => setAccessLevel(e.target.value as 'full' | 'workspace' | 'readonly' | 'observer')}
              className="mt-1 w-full rounded border border-[var(--border)] bg-[var(--card)] px-2 py-1.5 text-xs"
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button
              onClick={submit}
              disabled={busy || !botId.trim() || !secret.trim() || !isAbsolutePath(workspacePath)}

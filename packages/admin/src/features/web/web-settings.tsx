...
import { Plus, Trash2, Power, PowerOff, Edit3, Check, X, Loader2, Copy } from 'lucide-react'
...
export function WebSettings() {
  ...
  const reload = useCallback(() => {
    setLoading(true)
    api.web.listAccounts().then((r) => setAccounts(r.accounts)).catch((err) => {
      console.error('[web] list failed:', err)
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between border-b border-[var(--border)] px-4 py-3">
        <div>
          <h2 className="text-sm font-medium text-[var(--foreground)]">{t('web.title')}</h2>
          <p className="text-[11px] text-[var(--muted-foreground)]">{t('web.desc')}</p>
        </div>
        <button
...
function AccountRow(props: {
  ...
  async function save() {
    setBusy(true)
    try {
      await api.web.updateAccount(account.accountId, { label, workspacePath, accessLevel, language })
      onSaved()
    } catch (err) {
      alert(t('web.saveFailed', { error: err instanceof Error ? err.message : String(err) }))
  ...
  async function toggle() {
    setBusy(true)
    try {
      await api.web.updateAccount(account.accountId, { enabled: !account.enabled })
      onToggled()
    } catch (err) {
      alert(t('web.switchFailed', { error: err instanceof Error ? err.message : String(err) }))
    } finally { setBusy(false) }
  }

  async function remove() {
    if (!(await confirmAction(t('web.confirmDelete')))) return
    setBusy(true)
    try {
      await api.web.deleteAccount(account.accountId)
      onDeleted()
    } catch (err) {
      alert(t('web.deleteFailed', { error: err instanceof Error ? err.message : String(err) }))
    } finally { setBusy(false) }
  }
    ...
    return (
      <li className="px-4 py-3">
        <div className="space-y-2">
          <div>
            <label className="text-[10px] text-[var(--muted-foreground)]">{t('web.label')}</label>
            <input
              value={label}
              onChange={(e) => setLabel(e.target.value)}
              className="mt-0.5 w-full rounded border border-[var(--border)] bg-[var(--card)] px-2 py-1 text-xs"
              placeholder={t('web.labelPlaceholder')}
            />
          </div>
          <div>
            <label className="text-[10px] text-[var(--muted-foreground)]">{t('web.workspace')}</label>
            <input
              value={workspacePath}
          ...
          <div className="flex gap-2">
            ...
            >
              <Check className="h-3 w-3" /> {t('web.save')}
            </button>
            <button
            ...
            >
              <X className="h-3 w-3" /> {t('web.cancel')}
            </button>
          </div>
        </div>
      </li>
    ...
    <li className="flex items-center gap-3 px-4 py-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 flex items-center gap-2">
          <p className="truncate font-mono text-[10px] text-[var(--muted-foreground)]">
            ...
            {account.workspaceMissing && <span className="ml-1 text-red-400">{t('web.pathMissing')}</span>}
          </p>
        </div>
      ...
      <div className="flex items-center gap-1">
        <button
          onClick={toggle}
          disabled={busy}
          title={account.enabled ? t('web.disable') : t('web.enable')}
...
function AddDialog(props: { onClose: () => void; onDone: () => void }) {
  ...
  async function submit() {
    ...
    setBusy(true)
    try {
      const res = await api.web.createAccount({ workspacePath, label: label || undefined, accessLevel, language })
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-sm font-medium text-[var(--foreground)]">{t('web.createTitle')}</h3>
        ...
        <div className="space-y-3">
          <div>
            <label className="text-[11px] text-[var(--muted-foreground)]">{t('web.bindWorkspace')}</label>
            <input
              value={workspacePath}
              onChange={(e) => setWorkspacePath(e.target.value)}
          ...
          <div>
            <label className="text-[11px] text-[var(--muted-foreground)]">{t('web.nameOptional')}</label>
            <input
              value={label}
              onChange={(e) => setLabel(e.target.value)}
          ...
          <div>
            <label className="text-[11px] text-[var(--muted-foreground)]">{t('common.language')}</label>
            <select
              value={language}

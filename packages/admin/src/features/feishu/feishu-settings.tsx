...
export function FeishuSettings() {
  ...
  const reload = useCallback(() => {
    setLoading(true)
    api.feishu.listAccounts().then((r) => setAccounts(r.accounts)).catch((err) => {
      console.error('[feishu] list failed:', err)
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between border-b border-[var(--border)] px-4 py-3">
        <div>
          <h2 className="text-sm font-medium text-[var(--foreground)]">{t('feishu.title')}</h2>
          <p className="text-[11px] text-[var(--muted-foreground)]">{t('feishu.desc')}</p>
        </div>
        <button
...
function AccountRow(props: {
  ...
  async function save() {
    setBusy(true)
    try {
      await api.feishu.updateAccount(account.accountId, { label, workspacePath, accessLevel, language })
      onSaved()
    } catch (err) {
      alert(t('feishu.saveFailed', { error: err instanceof Error ? err.message : String(err) }))
  ...
  async function toggle() {
    setBusy(true)
    try {
      await api.feishu.updateAccount(account.accountId, { enabled: !account.enabled })
      onToggled()
    } catch (err) {
      alert(t('feishu.switchFailed', { error: err instanceof Error ? err.message : String(err) }))
    } finally { setBusy(false) }
  }

  async function remove() {
    if (!(await confirmAction(t('feishu.confirmDelete', { name: account.appId })))) return
    setBusy(true)
    try {
      await api.feishu.deleteAccount(account.accountId)
      onDeleted()
    } catch (err) {
      alert(t('feishu.deleteFailed', { error: err instanceof Error ? err.message : String(err) }))
    } finally { setBusy(false) }
  }
    ...
    return (
      <li className="px-4 py-3">
        <div className="space-y-2">
          <div>
            <label className="text-[10px] text-[var(--muted-foreground)]">{t('feishu.label')}</label>
            <input
              value={label}
              onChange={(e) => setLabel(e.target.value)}
              className="mt-0.5 w-full rounded border border-[var(--border)] bg-[var(--card)] px-2 py-1 text-xs"
              placeholder={t('feishu.labelPlaceholder')}
            />
          </div>
          <div>
            <label className="text-[10px] text-[var(--muted-foreground)]">{t('feishu.workspace')}</label>
            <input
              value={workspacePath}
          ...
          <div>
            ...
            >
              ...
              <option value="workspace">{t('feishu.wsWrite')}</option>
              <option value="full">{t('feishu.full')}</option>
            </select>
          </div>
          ...
          <div className="flex gap-2">
            ...
            >
              <Check className="h-3 w-3" /> {t('feishu.save')}
            </button>
            <button
            ...
            >
              <X className="h-3 w-3" /> {t('feishu.cancel')}
            </button>
          </div>
        </div>
      </li>
    ...
    <li className="flex items-center gap-3 px-4 py-3">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <span className="text-xs font-medium text-[var(--foreground)]">{account.label || account.appId}</span>
          <span className={cn(
        ...
        <p className="mt-0.5 truncate font-mono text-[10px] text-[var(--muted-foreground)]">
          app:{account.appId} bot:{account.botOpenId} → {account.workspacePath}
          {account.workspaceMissing && <span className="ml-1 text-red-400">{t('feishu.pathMissing')}</span>}
        </p>
      </div>
      <div className="flex items-center gap-1">
        <button
          onClick={toggle}
          disabled={busy}
          title={account.enabled ? t('feishu.disable') : t('feishu.enable')}
...
function AddDialog(props: { onClose: () => void; onDone: () => void }) {
  ...
  async function submit() {
    ...
    setBusy(true)
    try {
      await api.feishu.createAccount({
        appId, appSecret,
        verificationToken: verificationToken || undefined,
        encryptKey: encryptKey || undefined,
        workspacePath, label: label || undefined, accessLevel, language,
      })
      onDone()
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div
        className="w-[480px] max-h-[90vh] overflow-y-auto rounded-lg border border-[var(--border)] bg-[var(--background)] p-5 shadow-xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-sm font-medium text-[var(--foreground)]">{t('feishu.addTitle')}</h3>
        ...
        <div className="space-y-3">
          <div>
            <label className="text-[11px] text-[var(--muted-foreground)]">{t('feishu.appIdLabel')}</label>
            <input
              value={appId}
              onChange={(e) => setAppId(e.target.value)}
              className="mt-1 w-full rounded border border-[var(--border)] bg-[var(--card)] px-2 py-1.5 font-mono text-xs"
              placeholder="cli_xxxxxxx"
            />
          </div>
          <div>
            <label className="text-[11px] text-[var(--muted-foreground)]">{t('feishu.appSecretLabel')}</label>
            <input
              type="password"
              value={appSecret}
          ...
          <div>
            <label className="text-[11px] text-[var(--muted-foreground)]">{t('feishu.verificationTokenLabel')}</label>
            <input
          ...
          <div>
            <label className="text-[11px] text-[var(--muted-foreground)]">{t('feishu.encryptKeyLabel')}</label>
            <input
              type="password"
              value={encryptKey}
          ...
          <div>
            <label className="text-[11px] text-[var(--muted-foreground)]">{t('feishu.bindWorkspace')}</label>
            <input
              value={workspacePath}
              onChange={(e) => setWorkspacePath(e.target.value)}
              className="mt-1 w-full rounded border border-[var(--border)] bg-[var(--card)] px-2 py-1.5 font-mono text-xs"
          ...
          <div>
            <label className="text-[11px] text-[var(--muted-foreground)]">{t('feishu.nameOptional')}</label>
            <input
              value={label}
              onChange={(e) => setLabel(e.target.value)}
              className="mt-1 w-full rounded border border-[var(--border)] bg-[var(--card)] px-2 py-1.5 text-xs"
              placeholder={t('feishu.namePlaceholder')}
            />
          </div>
          <div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button
              onClick={submit}
              disabled={busy || !appId.trim() || !appSecret.trim() || !isAbsolutePath(workspacePath)}

...
export function SlackSettings() {
  ...
  const reload = useCallback(() => {
    setLoading(true)
    api.slack.listAccounts().then((r) => setAccounts(r.accounts)).catch((err) => {
      console.error('[slack] list failed:', err)
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between border-b border-[var(--border)] px-4 py-3">
        <div>
          <h2 className="text-sm font-medium text-[var(--foreground)]">{t('slack.title')}</h2>
          <p className="text-[11px] text-[var(--muted-foreground)]">{t('slack.desc')}</p>
        </div>
        <button
...
function AccountRow(props: {
  ...
  async function save() {
    setBusy(true)
    try {
      await api.slack.updateAccount(account.accountId, { label, workspacePath, accessLevel, language })
      onSaved()
    } catch (err) {
      alert(t('slack.saveFailed', { error: err instanceof Error ? err.message : String(err) }))
  ...
  async function toggle() {
    setBusy(true)
    try {
      await api.slack.updateAccount(account.accountId, { enabled: !account.enabled })
      onToggled()
    } catch (err) {
      alert(t('slack.switchFailed', { error: err instanceof Error ? err.message : String(err) }))
    } finally { setBusy(false) }
  }

  async function remove() {
    if (!(await confirmAction(t('slack.confirmDelete', { name: account.teamId })))) return
    setBusy(true)
    try {
      await api.slack.deleteAccount(account.accountId)
      onDeleted()
    } catch (err) {
      alert(t('slack.deleteFailed', { error: err instanceof Error ? err.message : String(err) }))
    } finally { setBusy(false) }
  }
    ...
    return (
      <li className="px-4 py-3">
        <div className="space-y-2">
          <div>
            <label className="text-[10px] text-[var(--muted-foreground)]">{t('slack.label')}</label>
            <input
              value={label}
              onChange={(e) => setLabel(e.target.value)}
              className="mt-0.5 w-full rounded border border-[var(--border)] bg-[var(--card)] px-2 py-1 text-xs"
              placeholder={t('slack.labelPlaceholder')}
            />
          </div>
          <div>
            <label className="text-[10px] text-[var(--muted-foreground)]">{t('slack.workspace')}</label>
            <input
              value={workspacePath}
          ...
          <div>
            <label className="text-[10px] text-[var(--muted-foreground)]">{t('slack.accessLevel')}</label>
            <select
              value={accessLevel}
              onChange={(e) => setAccessLevel(e.target.value as 'full' | 'workspace' | 'readonly' | 'observer')}
              className="mt-0.5 w-full rounded border border-[var(--border)] bg-[var(--card)] px-2 py-1 text-xs"
            >
              <option value="readonly">{t('slack.readonly')}</option>
              <option value="workspace">{t('slack.wsWrite')}</option>
              <option value="full">{t('slack.full')}</option>
            </select>
          </div>
          ...
          <div className="flex gap-2">
            ...
            >
              <Check className="h-3 w-3" /> {t('slack.save')}
            </button>
            <button
            ...
            >
              <X className="h-3 w-3" /> {t('slack.cancel')}
            </button>
          </div>
        </div>
      </li>
    ...
    <li className="flex items-center gap-3 px-4 py-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 truncate font-mono text-[10px] text-[var(--muted-foreground)]">
          team:{account.teamId} bot:{account.botUserId} → {account.workspacePath}
          {account.workspaceMissing && <span className="ml-1 text-red-400">{t('slack.pathMissing')}</span>}
        </p>
      </div>
      <div className="flex items-center gap-1">
        <button
          onClick={toggle}
          disabled={busy}
          title={account.enabled ? t('slack.disable') : t('slack.enable')}
...
function AddDialog(props: { onClose: () => void; onDone: () => void }) {
  ...
  async function submit() {
    ...
    setBusy(true)
    try {
      await api.slack.createAccount({ botToken, appToken, workspacePath, label: label || undefined, accessLevel, language })
      onDone()
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div
        className="w-[480px] rounded-lg border border-[var(--border)] bg-[var(--background)] p-5 shadow-xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-sm font-medium text-[var(--foreground)]">{t('slack.addTitle')}</h3>
        ...
        <div className="space-y-3">
          <div>
            <label className="text-[11px] text-[var(--muted-foreground)]">{t('slack.botTokenLabel')}</label>
            <input
              value={botToken}
              ...
              placeholder="xoxb-..."
            />
            <p className="mt-1 text-[10px] text-[var(--muted-foreground)]">{t('slack.botTokenHint')}</p>
          </div>
          <div>
            <label className="text-[11px] text-[var(--muted-foreground)]">{t('slack.appTokenLabel')}</label>
            <input
              value={appToken}
              ...
              placeholder="xapp-..."
            />
            <p className="mt-1 text-[10px] text-[var(--muted-foreground)]">{t('slack.appTokenHint')}</p>
          </div>
          <div>
            <label className="text-[11px] text-[var(--muted-foreground)]">{t('slack.bindWorkspace')}</label>
            <input
              value={workspacePath}
              onChange={(e) => setWorkspacePath(e.target.value)}
              className="mt-1 w-full rounded border border-[var(--border)] bg-[var(--card)] px-2 py-1.5 font-mono text-xs"
          ...
          <div>
            <label className="text-[11px] text-[var(--muted-foreground)]">{t('slack.nameOptional')}</label>
            <input
              value={label}
              onChange={(e) => setLabel(e.target.value)}
          ...
          <div>
            <label className="text-[11px] text-[var(--muted-foreground)]">{t('slack.accessLevel')}</label>
            <select
              value={accessLevel}
              onChange={(e) => setAccessLevel(e.target.value as 'full' | 'workspace' | 'readonly' | 'observer')}
              className="mt-1 w-full rounded border border-[var(--border)] bg-[var(--card)] px-2 py-1.5 text-xs"
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button
              onClick={submit}
              disabled={busy || !botToken.trim() || !appToken.trim() || !isAbsolutePath(workspacePath)}

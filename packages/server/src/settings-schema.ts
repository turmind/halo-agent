...
function generalSection(): SchemaSection {
  ...
  return {
    ...
    fields: ([
      ...
      { key: 'evolution.level', type: 'enum', options: ['L0', 'L1'], globalOnly: true, description: 'Self-evolution capability ladder. L0: disabled. L1: human + LLM assist (drafts via /note + pre-compact, manual approve/reject).', description_zh: '自我进化等级。L0：关闭。L1：人 + LLM 辅助（/note 与 pre-compact 起草，手动 approve/reject）。', default: 'L0' },
      ...
      { key: 'evolution.run_timeout_minutes', type: 'int', globalOnly: true, description: 'Heartbeat timeout for run wrappers; older than this with status=running is marked timeout.', description_zh: 'run wrapper 心跳超时（分钟）；超过仍标 running 的记录会被标 timeout。', default: '5' },
      ...
      { key: 'evolution.triggers.pre_compact', type: 'boolean', globalOnly: true, description: 'Snapshot the session right before compaction so evo can analyze it. /note is always available regardless of this setting.', description_zh: '在 compact 前快照会话供 evo 分析。/note 总是可用，与此开关无关。', default: 'true' },
      ...
      { key: 'limits.tool_result_render_chars', type: 'int', globalOnly: true, description: 'Per-tool-result UI render cap (admin chat panel). Larger results are truncated for display only — the LLM still sees the full content.', description_zh: '每条 tool result 在 admin UI 上的渲染长度上限（仅影响展示，LLM 看到的仍是完整内容）。', default: '8000' },

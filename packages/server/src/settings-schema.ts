...
function generalSection(): SchemaSection {
  ...
  return {
    ...
    fields: ([
      ...
      { key: 'evolution.level', type: 'enum', options: ['L0', 'L1'], globalOnly: true, description: 'Self-evolution capability ladder. L0: manual only — drafts via /note, manual approve/reject. L1: L0 plus automatic drafting on pre-compact. (Queued runs always execute regardless of level; the level only gates how they get enqueued.)', description_zh: '自我进化等级。L0：仅手动——通过 /note 起草，手动 approve/reject。L1：在 L0 基础上，压缩前（pre-compact）自动起草。（已排队的 run 始终执行，level 只决定如何入队。）', default: 'L0' },
      ...
      { key: 'evolution.run_timeout_minutes', type: 'int', globalOnly: true, description: 'Heartbeat timeout for run wrappers; older than this with status=running is marked timeout. Must exceed the wrapper per-phase cap (10min) so a healthy slow phase is never wrongly requeued.', description_zh: 'run wrapper 心跳超时（分钟）；超过仍标 running 的记录会被标 timeout。须大于 wrapper 单阶段上限（10 分钟），否则正常的慢阶段会被误判重排。', default: '12' },

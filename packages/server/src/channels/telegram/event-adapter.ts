...
export class TelegramResponder {
  ...
  private async dispatchChunk(chunk: string): Promise<void> {
    ...
    if (text) {
      ...
      catch (err) { console.warn(`[Telegram] sendText failed: ${err instanceof Error ? err.message : String(err)}`) }
    }
    for (const p of mediaPaths) {
      try { await this.deps.sendMedia(p) }
      catch (err) { console.warn(`[Telegram] sendMedia ${p} failed: ${err instanceof Error ? err.message : String(err)}`) }
    }
  }
}

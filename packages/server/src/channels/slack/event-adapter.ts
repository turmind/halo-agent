...
export class SlackResponder {
  ...
  private async dispatchChunk(chunk: string): Promise<void> {
    ...
    if (text) {
      ...
      catch (err) { console.warn(`[Slack] sendText failed: ${err instanceof Error ? err.message : String(err)}`) }
    }
    for (const p of mediaPaths) {
      try { await this.deps.sendMedia(p) }
      catch (err) { console.warn(`[Slack] sendMedia ${p} failed: ${err instanceof Error ? err.message : String(err)}`) }
    }
  }
}

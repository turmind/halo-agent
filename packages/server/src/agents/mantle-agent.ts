...
export class MantleAgent extends AgentLoop {
  ...
  protected async callModel(signal: AbortSignal | undefined): Promise<ModelCallResult> {
    ...
    // Reasoning is opt-in. Live endpoint accepts low|medium|high|xhigh|max
    // (max works but isn't in the model's own validation-error list — see
    // aws-bedrock-mantle.yaml); anything else falls back to high so a
    // misconfigured agent doesn't 400.
    if (this.config.thinking?.enabled && this.config.thinking.effort) {
      ...
      body.reasoning = { effort: valid.includes(e) ? e : 'high' }
    }

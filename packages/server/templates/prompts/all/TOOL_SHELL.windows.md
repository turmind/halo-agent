...
- **Skill scripts**: some built-in skills ship `.py` helpers (e.g. cron, ws, acp) and assume unix. If `python` isn't found, install Python first; the ws skill's `stage.py` additionally needs `pip install pyyaml`. Skip any `chmod +x` step — it's a no-op on Windows.

...
- Channels: after a server restart, WeChat and Telegram replies from a session that resumed on its own (the run-ledger restart nudge, a queued turn, a message typed in the admin) were dropped until the user wrote to the bot again — the reply route only existed in memory and was only created by an inbound message. Each account now re-wires, at start, the latest existing session of every user it can address from the account row: WeChat users with a stored `context_token`, and the Telegram `lastActiveChatId` when it is a private chat (a group chat id doesn't say whose session it was). No session is created, and a workspace whose runtime another live server owns is skipped. Failed WeChat / Telegram reply sends now log at `warn` instead of `info`, so they show up at the default log level.

...
- **Packaging / release: read the deploy docs FIRST**: Before any build, bundle, or publish (npm / desktop dmg / Windows exe / dev-server deploy), read `.halo/docs/dev/desktop-packaging.md` (Gotchas section) + `deploy.md` — do NOT package from memory. Non-negotiable checklist, each learned from a shipped regression:
  ...
  - **A release is not done until the Windows exe is attached to the GitHub release.** Default scope of "发版 / 打包发布" = npm publish → git tag + push → `gh release create` → `HALO_STAGE_FULL=1 CI=true pnpm dist:win` → `gh release upload v<ver> "packages/desktop/dist/Halo Setup <ver>.exe"` (the macOS dmg is the user's own step). Both v1.3.0 and v1.3.1 shipped without the exe because the orchestrator ran the release from memory and stopped at `gh release create`.
  - **Always hand releases to the `release` agent, even when the steps feel fresh** — it reads the deploy docs, the orchestrator doesn't; "I remember the checklist" is exactly the state in which steps get skipped. The orchestrator's job is the brief (what's already done, what's left, hard fences) and the post-check: `gh release view v<ver> --json assets` must list the exe.
  - When delegating packaging to a sub-agent, paste this checklist into the brief — the regressions above all happened because a packaging step was skipped silently.
...
- **When the brief would be longer than the diff, don't delegate — do it.** If the diagnosis is already done and the change is ≲30 lines in one or two files (plus their test), the orchestrator edits directly and runs the scoped `tsc` + vitest itself; a sub-session there costs more than it saves. This applies to *code* only — packaging / release is never "small enough" (see Packaging / release above).

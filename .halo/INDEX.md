...
Scheduled agent runs (cross-workspace) from the **Cron** admin tab. A user defines `(workspace, agent, prompt, schedule-or-runAt, channel-targets, optional session)`; the server schedules via croner; on fire, a `halo cli` child runs the prompt in one stable session — `cron-<jobId>` by default, or an existing root session picked on the job (which keeps its own agent and access level; a fire is skipped while a turn runs there) — created on first run, resumed on subsequent runs so the conversation accumulates over time and the user can review history in the Sessions tab; captured stdout fans out to bound channel accounts and the run is recorded in an audit log. UI updates are pushed via WS (`cron:job_changed` / `cron:run_changed`) — no client polling. Two trigger modes: **recurring** (5-field cron expression) and **one-shot at-mode** (`runAt` epoch ms, auto-disables after fire). See [design/cron.md](docs/design/cron.md) for the full design; early proposal notes in `plans/cron-tasks.md` (local-only).
...
Key state:
- `~/.halo/global/cron.db` — global tables `cron_jobs` (with `run_at` column for at-mode, nullable `session_id` for a picked session) + `cron_runs`
...
Halo server as an **Amazon Bedrock AgentCore Runtime** container: `HALO_RUNTIME_MODE=agentcore` skips the password/JWT gate (auth terminates upstream at AgentCore via SigV4 presign), the single-instance lock, channels, cron and evolution — leaving the AgentCore contract surface: `GET /ping` (`HealthyBusy` while any agent session runs — the official keep-alive), `POST /invocations`, and streaming `WS /ws`. Each runtime session id maps to an isolated per-user workspace `<HALO_WORKSPACE>/users/<id>/` (own sqlite + session files) on EFS, so microVM termination loses nothing: next connect cold-starts (~3s) and history reloads. The WS proxy only forwards `inputText` frames — the frontend sends `{"inputText":"/init"}` on open to pull history, plus a 30s keepalive ping that (because an open WS counts as an in-flight invocation) keeps the whole session warm.

Driven from `packages/server/src/routes/agentcore.ts` (adapter) + `packages/agentcore-demo/` (Dockerfile, chat frontend, CDK stack, auth/presign Lambdas — README has the ops gotchas). See [design/agentcore.md](docs/design/agentcore.md).
